...
// Inline unit tests removed; tests moved to `/tests/` integration files.
...
#[update]
fn admin_reset_stable_storage() -> Result<(), LedgerError> {
    // Validate admin authentication
...
// Internal helper performing reset, callable from unit tests.
fn reset_stable_storage_internal(admin: Principal) -> Result<(), LedgerError> {
    // Clear stable maps
    ADMIN_STORAGE.with(|a| {
        let keys: Vec<u8> = a.borrow().keys().collect();
        for k in keys {
            a.borrow_mut().remove(&k);
        }
        a.borrow_mut().insert(0, admin);
    });

    METADATA.with(|m| {
        let keys: Vec<u8> = m.borrow().keys().collect();
        for k in keys {
    ...
    BALANCES.with(|b| {
        let keys: Vec<Account> = b.borrow().keys().map(|k| k.clone()).collect();
        for k in keys {
            b.borrow_mut().remove(&k);
        }
    });

    ALLOWANCES.with(|al| {
        let keys: Vec<AllowanceKey> = al.borrow().keys().map(|k| k.clone()).collect();
        for k in keys {
            al.borrow_mut().remove(&k);
        }
    });

    ALLOWANCE_EXPIRATIONS.with(|ae| {
        let keys: Vec<AllowanceKey> = ae.borrow().keys().map(|k| k.clone()).collect();
        for k in keys {
            ae.borrow_mut().remove(&k);
        }
    });

    REFERRAL_BY_ACCOUNT.with(|r| {
        let keys: Vec<Account> = r.borrow().keys().map(|k| k.clone()).collect();
        for k in keys {
            r.borrow_mut().remove(&k);
        }
    });

    ACCOUNT_BY_REFERRAL.with(|a| {
        let keys: Vec<String> = a.borrow().keys().map(|k| k.clone()).collect();
        for k in keys {
            a.borrow_mut().remove(&k);
        }
    });

    CLAIMED_REFERRALS.with(|c| {
        let keys: Vec<Account> = c.borrow().keys().map(|k| k.clone()).collect();
        for k in keys {
    ...
    TRANSACTIONS.with(|t| {
        let keys: Vec<[u8; 32]> = t.borrow().keys().collect();
        for k in keys {
            t.borrow_mut().remove(&k);
        }
    });

    LOGS.with(|l| {
        let keys: Vec<u64> = l.borrow().keys().collect();
        for k in keys {
            l.borrow_mut().remove(&k);
        }
    });
    ...
    // Reset cells
    LOG_COUNTER.with(|c| c.borrow_mut().set(0).expect("Failed to reset LOG_COUNTER"));
    ADMIN_BYPASS_ENABLED.with(|ab| {
        ab.borrow_mut()
            .set(false)
